...
class ForceField(corner1: Vector3d, corner2: Vector3d) {
    ...
    val center : Vector3d = Vector3d(lowerCorner.world, (lowerCorner.x + upperCorner.x) / 2, (lowerCorner.y + upperCorner.y) / 2, (lowerCorner.z + upperCorner.z) / 2)
    var on3dInside: (Player) -> Unit = { _ -> }
    var on3dOutSide: (Player) -> Unit = { _ -> }
    var on2dInside: (Player) -> Unit = { _ -> }
    var on2dOutSide: (Player) -> Unit = { _ -> }

    constructor(center: Vector3d, radius: Int) : this(
        Vector3d(center.x - radius, center.y - radius, center.z - radius),
        Vector3d(center.x + radius, center.y + radius, center.z + radius)
    )
}