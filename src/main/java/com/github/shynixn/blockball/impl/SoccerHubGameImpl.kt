...
class SoccerHubGameImpl(
    ...
    private fun prepareLobbyStorageForPlayer(player: Player, team: Team, teamMeta: TeamMeta) {
        ...
        val message = if(team == Team.RED) {
            placeHolderService.replacePlaceHolders(language.joinTeamRedMessage, player, this, teamMeta, 0)
        }else{
            placeHolderService.replacePlaceHolders(language.joinTeamBlueMessage, player, this, teamMeta, 0)
        }
